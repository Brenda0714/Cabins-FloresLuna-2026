...
// Diccionario simplificado basado en tus Signals de Angular (Sin imágenes)
const infoCabanas = {
  "Orquídea": {
...
// Buscamos los datos extras por el nombre de la cabaña
...
// Si el usuario existe, tomamos su ID real directamente de la base de datos
    ...
    db.query(queryReserva, [usuarioId, cabin_nombre, fecha_llegada, fecha_salida, noches, precioUnitario, monto_total], (err, reservaResult) => {
      ...
      // ==============================================================================================
      // 💳 SIMULACIÓN DE INSERCIÓN EN LA TABLA DE PAGOS (PAYPAL)
      // ==============================================================================================
      const idDeLaReservaCreada = reservaResult.insertId; // 👈 Extraemos el ID recién generado
      const folioSimulado = 'FL-' + Math.floor(Math.random() * 900000 + 100000); // Ejemplo: FL-583921
      const pagoExitoso = Math.random() > 0.10;
      let estadoPagoDB = 'completado';
      const referenciaPayPalSimulada = 'PAYID-' + Math.random().toString(36).substring(2, 17).toUpperCase(); // Ejemplo: PAYID-A8F3K9...
      let badgeColor = '#e8f5e9';
      let badgeTextoColor = '#2e7d32';
      let badgeTexto = 'Completado';

      // 🚨 Si la condición simula que el pago NO pasó:
      if (!pagoExitoso) {
        estadoPagoDB = 'fallido'; // Cambia al valor de tu ENUM
        referenciaPayPalSimulada = 'N/A (Transacción Rechazada)';
        badgeColor = '#ffebee';
      ...
      const queryPago = `
        INSERT INTO pagos (reserva_id, folio, monto, metodo_pago, estado_pago, referencia_pago)
        VALUES (?, ?, ?, 'PayPal', 'completado', ?)
      `;

      db.query(queryPago, [idDeLaReservaCreada, folioSimulado, monto_total, referenciaPayPalSimulada], (pagoErr, pagoResult) => {
        if (pagoErr) {
          // Nota: Si el pago falla pero la reserva se creó, lo registramos en consola para no romperle el flujo al cliente
          console.error('❌ Error al insertar en la tabla pagos:', pagoErr);
        } else {
          console.log(`💰 ¡Pago simulado e insertado con éxito! Folio: ${folioSimulado} para Reserva ID: ${idDeLaReservaCreada}`);
        }


      // ==============================================================================================
      // 📧 ENVÍO DE CORREOS (Se ejecuta después de procesar la base de datos)
      // ==============================================================================================
      ...
      const htmlCliente = `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #fcfaf7; padding: 30px 15px; color: #4a3e3d;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 25px rgba(139, 69, 19, 0.05); border: 1px solid #f3e9dc;">
            ...
            <!-- Encabezado -->
            <div style="background-color: #ba4a23; padding: 35px 30px; text-align: center;">
              <h1 style="color: #ffffff; margin: 0; font-size: 26px; font-weight: 700; letter-spacing: 1px;">Flores de la Luna</h1>
            ...
            <!-- ==============================================================================================
                🌲 NUEVA SECCIÓN: DETALLES DE LA CABAÑA (BASADO EN TU SIGNAL DE ANGULAR)
                ============================================================================================== -->
            <div style="padding: 0 30px 15px 30px;">
              <div style="background-color: #ffffff; border: 1px solid #f5eadd; border-radius: 16px; padding: 22px; box-shadow: 0 4px 12px rgba(92, 44, 22, 0.02);">
                ...
                <h3 style="margin: 0 0 4px 0; color: #5c2c16; font-size: 17px; font-weight: 700;">
                  🏡 Detalles del hospedaje: Cabaña ${cabin_nombre}
                </h3>

                <p style="margin: 0 0 12px 0; color: #ba4a23; font-size: 12px; font-weight: bold; letter-spacing: 0.5px; text-transform: uppercase;">
                  ${datosExtraCabana.caracteristicas}
                </p>

                <p style="margin: 0 0 16px 0; color: #6b5b55; font-size: 14px; line-height: 1.6;">
                  ${datosExtraCabana.descripcion}
                </p>

                <div style="background-color: #fdfbf9; padding: 14px 18px; border-radius: 10px; border-left: 4px solid #ba4a23; border: 1px solid #fbf4eb; border-left: 4px solid #ba4a23;">
                  <span style="font-size: 11px; font-weight: 800; color: #ba4a23; display: block; margin-bottom: 6px; letter-spacing: 0.5px; text-transform: uppercase;">
                    Servicios e Instalaciones Incluidas:
                  </span>
                  <p style="margin: 0; color: #4a3e3d; font-size: 13px; line-height: 1.5; font-weight: 500;">
                    ${datosExtraCabana.amenidades}
                  </p>
                </div>
            ...
            <div style="padding: 15px 30px 35px 30px;">
              <div style="border: 2px dashed #d5c3b2; background-color: #fffdfb; border-radius: 14px; padding: 22px;">
                ...
                <table style="width: 100%; font-size: 13px; color: #6b5b55; line-height: 1.9;">
                  ...
                  <tr>
                    <td colspan="2" style="padding-top: 15px; border-bottom: 1px dashed #ebdccb;"></td>
                  </tr>

                  <tr>
                    <td style="padding-top: 15px; color: #5c2c16; font-size: 15px; font-weight: bold;">Monto Total:</td>
                    <td style="padding-top: 15px; text-align: right; color: #ba4a23; font-size: 20px; font-weight: 800;">$${monto_total} MXN</td>
                  </tr>
                </table>
              </div>
            </div>
            ...
            <div style="background-color: #fdfbf9; padding: 25px; text-align: center; border-top: 1px solid #f5eadd; font-size: 12px; color: #8e7a74;">
              ...
              <p style="margin: 0; font-weight: bold; color: #ba4a23; font-size: 13px;">¡Te esperamos pronto en Flores de la Luna!</p>
            </div>
      ...
      const htmlAdmin = `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #fcfaf7; padding: 30px 15px; color: #4a3e3d;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #f3e9dc;">
            ...
            <div style="padding: 0 30px 15px 30px;">
              <h3 style="color: #ba4a23; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 10px 0; border-bottom: 1px solid #f5eadd; padding-bottom: 5px;">Datos del Huésped</h3>
              <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                <tr>
                  <td style="padding: 8px 0; color: #8e7a74; width: 35%;">Nombre completo:</td>
                  <td style="padding: 8px 0; color: #2d2522; font-weight: 600;">${nombre}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; color: #8e7a74;">Correo electrónico:</td>
                  <td style="padding: 8px 0; color: #ba4a23; font-weight: 600; font-style: italic;">${email}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; color: #8e7a74;">Teléfono de contacto:</td>
                  <td style="padding: 8px 0; color: #2d2522; font-weight: 600;">${telefono || 'No proporcionado'}</td>
                </tr>
              </table>
            </div>

            <div style="padding: 0 30px 30px 30px;">
              <h3 style="color: #ba4a23; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 10px 0; border-bottom: 1px solid #f5eadd; padding-bottom: 5px;">Detalles del Hospedaje</h3>

              <div style="background-color: #fdfbf9; border: 1px solid #f5eadd; border-radius: 12px; padding: 20px; margin-top: 10px;">
                <table style="width: 100%; border-collapse: collapse; font-size: 14px; line-height: 1.8;">
                  <tr>
                    <td style="font-weight: 600; color: #5c2c16;">Cabaña Solicitada:</td>
                    <td style="text-align: right; color: #2d2522; font-weight: bold; font-size: 15px;">${cabin_nombre}</td>
                  </tr>
                  <tr>
                    <td style="color: #6b5b55;">Fecha de Check-In:</td>
                    <td style="text-align: right; color: #2d2522; font-weight: 600;">${fecha_llegada}</td>
                  </tr>
                  <tr>
                    <td style="color: #6b5b55;">Fecha de Check-Out:</td>
                    <td style="text-align: right; color: #2d2522; font-weight: 600;">${fecha_salida}</td>
                  </tr>
                  <tr>
                    <td style="color: #6b5b55; padding-bottom: 10px; border-bottom: 1px dashed #ebdccb;">Total de Noches:</td>
                    <td style="text-align: right; color: #2d2522; font-weight: 600; padding-bottom: 10px; border-bottom: 1px dashed #ebdccb;">${noches} noches</td>
                  </tr>

                  <tr>
                    <td style="padding-top: 10px; color: #6b5b55;">Estatus Financiero:</td>
                    <td style="padding-top: 10px; text-align: right;">
                      <span style="background-color: ${badgeColor}; color: ${badgeTextoColor}; padding: 3px 12px; border-radius: 30px; font-size: 12px; font-weight: bold; text-transform: uppercase;">
                        ${badgeTexto}
                      </span>
                    </td>
                  </tr>
                  <tr>
                    <td style="color: #8e7a74; font-size: 12px;">ID de Transacción:</td>
                    <td style="text-align: right; font-family: monospace; font-size: 12px; color: #4a3e3d;">${referenciaPayPalSimulada}</td>
                  </tr>

                  <tr>
                    <td style="padding-top: 12px; font-weight: bold; color: #5c2c16; font-size: 15px;">Monto de la Operación:</td>
                    <td style="padding-top: 12px; text-align: right; color: ${pagoExitoso ? '#2e7d32' : '#c62828'}; font-weight: 800; font-size: 18px;">$${monto_total} MXN</td>
                  </tr>
                </table>
              </div>
            </div>
            ...
            <div style="padding: 0 30px 35px 30px; text-align: center;">
              <a href="http://localhost:4200/admin/dashboard" style="display: inline-block; background-color: #ba4a23; color: #ffffff; padding: 12px 25px; border-radius: 10px; font-weight: bold; text-decoration: none; font-size: 14px; box-shadow: 0 4px 10px rgba(186, 74, 35, 0.25);">
